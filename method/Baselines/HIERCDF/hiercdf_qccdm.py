...
class HierCDF(nn.Module):
    ...
    def __init__(self, n_user, n_item, n_know, hidden_dim, know_graph: pd.DataFrame, Q_matrix=None, device='cuda'):
        ...
        if Q_matrix is not None:
            if not isinstance(Q_matrix, torch.Tensor):
                self.q_mask = torch.tensor(Q_matrix).to(device)
            else:
                self.q_mask = Q_matrix
            self.q_mask.requires_grad = False
            self.q_neural = torch.randn(self.n_item, self.n_know).to(device)
    ...
    def forward(self, user_ids: torch.LongTensor, item_ids: torch.LongTensor, item_know: torch.Tensor,
        ...
        if self.Q_matrix is not None:
            user_factor = torch.tanh(
                self.user_contract(user_mastery * (self.q_neural * (1 - self.q_mask) + self.q_mask)[item_ids]))
            item_factor = torch.sigmoid(
                self.item_contract(item_diff * (self.q_neural * (1 - self.q_mask) + self.q_mask)[item_ids]))
        else:
            user_factor = torch.tanh(
...
class HierCDM(CDM):
    def __init__(self, n_user, n_item, n_know, hidden_dim, know_graph: pd.DataFrame, Q_matrix=None, device='cuda'):
        self.net = HierCDF(n_user, n_item, n_know, hidden_dim, know_graph, Q_matrix)
        ...
        for epoch_i in range(epoch):
            ...
            for batch_data in tqdm(train_data, "Epoch %s" % epoch_i):
                ...
                if self.Q_matrix is not None:
                    ...
                    l1_lambda = alpha / self.know_num / self.prob_num
                    q_loss = l1_lambda * l1_reg.abs().sum()
                    loss = loss_fn(torch.log(output), y_target, user_ids) + q_loss
                else:
                ...
                if self.net.Q_matrix is not None:
