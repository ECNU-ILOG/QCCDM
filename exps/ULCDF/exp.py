...
path_CDM_ULCDF_runner = path_CDM_ILOG + '\\runners\\ULCDF'
sys.path.insert(0, path_CDM_ILOG)
sys.path.insert(0, path_CDM_ULCDF_runner)
from data.data_params_dict import data_params
from runners.ULCDF.utils import data_params, epochs_dict
from runners.ULCDF.cdm_runners import get_runner
...
parser.add_argument('--method', default='ulcdf', type=str, help='A Lightweight Graph-based Cognitive Diagnosis Framework', required=True)
...
parser.add_argument('--keep_prob', type=float, default=0.9, help='edge drop probability')
parser.add_argument('--leaky', type=float, default=0.5, help='negative slope of LeakyRelu')
parser.add_argument('--noise_ratio', type=float, help='the proportion of noise which added into response logs')
parser.add_argument('--bad_ratio', type=float, help='the proportion of drop bad solely for junyi')
...
pprint(config_dict)
run = wb.init(project="ULCDF", name=name,
              tags=tags,
              config=config_dict)
